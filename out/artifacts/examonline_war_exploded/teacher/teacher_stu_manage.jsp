...
<body>
	...
	<table>
		<caption>
			学生信息管理 <span>（共有${requestScope.total}名学生）</span>
			<a href="${basepath}/BatchOutServlet?mode=1" id="btn_excel" type="button">下载本表</a>
		</caption>
		<tr>
			<form action="${basepath}/ShowStuPage">
				<td><b> 学号 <input type="text" name="s_ID" size="10"
						placeholder="查询学号"> <input type="submit" value="查询"></b></td>
			</form>
			<td><b>姓名</b></td>
			<td><b>班级</b></td>
			<td><b>成绩</b></td>
		...
		<c:forEach var="row" items="${requestScope.stuTableList}"
			...
			<c:if test="${param.modify_id != row.ID}" var="flag"
				...
				<form action="" method="post" name="f${row.ID}"
					...
					<td><input type=button value="修改"
						...
						onclick="javascript:if(del()){f${row.ID}.action='HandleStuInfo?executeMode=2&ID=${row.ID}&cpage=${requestScope.cpage}&per=${requestScope.per}';f${row.ID}.submit();}">
					</td>
					</tr>
				</form>
			</c:if>
			<c:if test="${not flag}">
				<form action="" method="post" name="f${row.ID}">
					...
					<td><input type="text" size="10" value="${row.password}"
						name="password" class="text1" /></td>
					<td><input type=button value="提交"
						onclick="f${row.ID}.action='HandleStuInfo?executeMode=1&cpage=${requestScope.cpage}&per=${requestScope.per}';f${row.ID}.submit()">
						&nbsp;&nbsp;&nbsp;&nbsp; <input type="button" value="删除"
	...
	<div style="margin-top:1em;text-align:center">
				...
				<c:choose>
					...
					<c:when test="${requestScope.totalPage<7}">
						<c:choose>
							...
							<c:otherwise>
								<%--其他页赋予相应的链接，通过get方法，给ShowStuPage生成相应的testtableList输出--%>
								<a href="./ShowStuPage?cpage=${page-1}&per=${requestScope.per}">${page}</a>
							</c:otherwise>
						</c:choose>
					</c:when>
					<c:otherwise>
						<c:choose>
							...
							<c:when
								...
								<c:choose>
									...
									<c:otherwise>
										<a
											href="./ShowStuPage?cpage=${page-1}&per=${requestScope.per}">${page}</a>
									</c:otherwise>
								</c:choose>
								<c:set var="count_points" value="0" />
							</c:when>
		...
		每页 <a href="./ShowStuPage?cpage=${requestScope.cpage}&per=1">1</a> <a
			href="./ShowStuPage?cpage=${requestScope.cpage}&per=5">5</a> <a
			href="./ShowStuPage?cpage=${requestScope.cpage}&per=10">10</a> <a
			href="./ShowStuPage?cpage=${requestScope.cpage}&per=20">20</a> <a
			href="./ShowStuPage?cpage=${requestScope.cpage}&per=30">30</a> <a
			href="./ShowStuPage?cpage=${requestScope.cpage}&per=${requestScope.total}">ALL</a>
		条记录
	</div>
