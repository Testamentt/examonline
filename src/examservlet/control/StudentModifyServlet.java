...
public class StudentModifyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String uri = "jdbc:mysql://127.0.0.1:3307/examination?user=root&password=&characterEncoding=utf-8&useSSL=false";
	...
	private void fail(HttpServletRequest request, HttpServletResponse response, String backNews) {
		...
		try {
			PrintWriter out = response.getWriter();
			out.println("<script language=javascript>alert('" + backNews + "');window.location='" + request.getContextPath() + "ShowStuPage';</script>");
