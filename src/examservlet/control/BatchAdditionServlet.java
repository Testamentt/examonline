...
public class BatchAdditionServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//截取不同类型的文件需要自行判断
		String fileName = cd.substring(cd.lastIndexOf("=")+2, cd.length()-1); // 获取part对象所携带的文件名称
		...
		for (Part imgPath : request.getParts()) { // 遍历上传的每一个part
			if (imgPath.getName().startsWith("img")) {
				...
				String cd1 = part.getHeader("Content-Disposition");
				//截取不同类型的文件需要自行判断
				String fileName1 = cd1.substring(cd1.lastIndexOf("=")+2, cd1.length()-1);; // 获取单张图片的文件名
